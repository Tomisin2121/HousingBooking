  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-surf px-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-line">
          ...
          <div className="bg-primary-50 border border-primary/10 rounded-xl p-3 text-xs text-ink-soft space-y-0.5">
            <p className="font-semibold text-primary-dark">Demo accounts (password: Password123!)</p>
    ...
    <div className="min-h-screen flex items-center justify-center bg-surf px-4 py-8">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-line">
          ...
          {type === 'student' && (
            <>
              <Input label="Matric Number" placeholder="FUT/2022/001" value={matricNo} onChange={(e) => setMatricNo(e.target.value)} required />
          ...
          {type === 'agent' && (
            <>
              ...
              <Input label="ID Document URL" placeholder="Cloudinary URL" value={idDocUrl} onChange={(e) => setIdDocUrl(e.target.value)} required />
              <p className="text-xs text-ink-soft bg-yellow-50 p-2 rounded-lg">
                Agent accounts require admin approval before you can post listings.
              </p>
            </>
          )}
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm p-6 space-y-4 border border-line">
          {done ? (
            <div className="text-center py-4">
              ...
              <p className="text-sm">If that email exists, a reset link has been sent.</p>
              <Link to="/login" className="text-primary text-sm font-medium mt-3 inline-block hover:underline">Back to Login</Link>
            </div>
          ) : (
            <>
              <Input
              ...
              <Link to="/login" className="text-sm text-primary font-medium hover:underline block text-center">
                Back to Login
              </Link>
            </>
          )}
