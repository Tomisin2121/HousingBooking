...
export function buildSeed(): DB {
  const student1: DBUser = {
    ...
    email: 'student@housingbooking.app', role: 'student', password: 'Password123!', created_at: iso(60 * 24 * 90),
  };
  const student2: DBUser = {
    id: 'u_student2', name: 'Aminat Yusuf', matric_no: 'FUT/2022/0451', department: 'Electrical Engineering',
    email: 'student2@housingbooking.app', role: 'student', password: 'Password123!', created_at: iso(60 * 24 * 60),
  };

  const agent1User: DBUser = { id: 'u_ag1', name: 'Chinedu Okafor', email: 'agent1@housingbooking.app', role: 'agent', password: 'Password123!', created_at: iso(60 * 24 * 90) };
  const agent2User: DBUser = { id: 'u_ag2', name: 'Bimbo Adeleke', email: 'agent2@housingbooking.app', role: 'agent', password: 'Password123!', created_at: iso(60 * 24 * 3) };
  const agent3User: DBUser = { id: 'u_ag3', name: 'Femi Akinola', email: 'agent3@housingbooking.app', role: 'agent', password: 'Password123!', created_at: iso(60 * 24 * 120) };

  const admin1: DBUser = { id: 'u_admin1', name: 'Happiness Ebuara', email: 'supresident@student.futa.edu.ng', role: 'admin', password: 'Password123!', created_at: iso(60 * 24 * 300) };

  const agents: Agent[] = [
    { id: 'ag_1', user_id: 'u_ag1', business_name: 'Alpha Hostels', phone: '0803 000 0001', verified: true, status: 'approved', created_at: iso(60 * 24 * 90) },
    { id: 'ag_2', user_id: 'u_ag2', business_name: 'GateView Estates', phone: '0803 000 0002', verified: false, status: 'pending', created_at: iso(60 * 24 * 3) },
    { id: 'ag_3', user_id: 'u_ag3', business_name: 'Campus Comfy', phone: '0803 000 0003', verified: true, status: 'approved', created_at: iso(60 * 24 * 120) },
  ];
  ...
  const msg = (id: string, convId: string, senderId: string, content: string, minsAgo: number, type: Message['type'] = 'text'): Message => ({
    id, conversation_id: convId, sender_id: senderId, content, type, created_at: iso(minsAgo),
  });
  ...
  return {
    users: [student1, student2, agent1User, agent2User, agent3User, admin1],
    agents,
    listings,
...
export async function hydrateMock() {
  try {
...
export function getCurrentUser(): DBUser | null {
  return currentUserId ? db.users.find((u) => u.id === currentUserId) ?? null : null;
}
...
export function signupStudentLocal(data: { name: string; email: string; password: string; matric_no?: string; department?: string }) {
  const user: DBUser = {
    ...
    role: 'student', matric_no: data.matric_no, department: data.department, created_at: iso(0),
  };
  db.users.push(user);
  persist();
  currentUserId = user.id;
  const { password: _pw, ...safeUser } = user;
  return { token: makeToken(user.id), user: safeUser, agent: null };
}
...
export function signupAgentLocal(data: { name: string; email: string; password: string; business_name: string; phone: string; id_document_url: string }) {
  const user: DBUser = {
    id: `u_${Math.random().toString(36).slice(2, 9)}`,
    name: data.name, email: data.email, password: data.password || 'Password123!', role: 'agent', created_at: iso(0),
  };
  const agent: Agent = {
    id: `ag_${Math.random().toString(36).slice(2, 9)}`,
    user_id: user.id, business_name: data.business_name, phone: data.phone,
    verified: false, status: 'pending', created_at: iso(0),
  };
  db.users.push(user);
...
export function listListingsLocal(params: Record<string, string> = {}): Listing[] {
  ...
  if (params.available && params.available !== 'false' && params.available !== 'all') out = out.filter((l) => l.available);
...
function inlineUserMsg(agentRecordId: string, content: string) {
  ...
  const conv = db.conversations.find((c) => {
    const listing = db.listings.find((l) => l.id === c.listing_id);
    return c.participants.includes(me.id) && listing?.agent_record_id === agentRecordId;
  });
  if (!conv) return;
...
export function sendMessageLocal(convId: string, content: string): Message {
  const me = getCurrentUser()!;
  const conv = db.conversations.find((c) => c.id === convId) ?? null;
  const msg: Message = {
    id: `m_${Math.random().toString(36).slice(2, 9)}`,
  ...
  persist();
  if (conv) scheduleAgentReply(convId);
  return msg;
}
...
function scheduleAgentReply(convId: string) {
  ...
  if (!conv) return;
  const peerId = conv.participants.find((p) => p !== getCurrentUser()?.id);
  if (!peerId) return;
  const agent = db.agents.find((a) => a.user_id === peerId);
  if (!agent) return;
...
// ─── Admin helpers (used by web; kept for parity) ───────────────────────────
...
export function allBookingsLocal() {
  return db.bookings;
}
